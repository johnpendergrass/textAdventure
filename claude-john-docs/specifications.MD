...
- **Game Status Information Display:**
  - **Scoring System (100 points total):**
    - CANDY COLLECTED (30 pts): Common candy (1 pt), Good candy (2 pts), Rare candy (5 pts)
    - SECRETS DISCOVERED (25 pts): 10 hidden areas/passages (2.5 pts each)
    - SPOOKY ENCOUNTERS (20 pts): Befriend house inhabitants (2 pts each, max 10)
    - ROOMS EXPLORED (15 pts): Visit all major rooms (1.5 pts per room, 10 rooms)
    - HEALTH/ENERGY (10 pts): Final health percentage × 0.1 = points
...
- **Current Data Files:**
  - `startGameText.json` - Game initialization content (renamed from samplePlaythrough.json)
  - `commands.json` - All game commands with type, shortcuts, and responses
  - `asciiArt.txt` - Text format ASCII art library with metadata (DEFAULT, CASTLE, SAMPLE, QUESTION)
  - `asciiArt.json` - Legacy JSON format ASCII art library
- **Command Structure:**
  ```json
  {
    "command_name": {
...
- **Simplified Rules System (for 10-room game):**
  - State flags approach instead of complex rules engine
  - Basic if/then logic for limited actions
  - JSON-embedded conditions (blocked exits, lighting requirements)
...
- `textGame.js` - Complete game engine with text buffer, command processing, input handling, ASCII art system
- `startGameText.json` - Game initialization content (renamed from samplePlaythrough.json)
- `commands.json` - All game commands with smart matching and responses
- `asciiArt.txt` - Text format ASCII art library with metadata parsing
- `asciiArt.json` - Legacy JSON format ASCII art library
